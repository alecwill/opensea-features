...
import ComputingButtons from './components/ComputingButtons'
...
const injectComputingButtons = async () => {
  const selectors = await fetchSelectors()
  const collectionMenu = document.querySelector(
    selectors.searchResults.menuSelector,
  ) as HTMLElement | null
  if (collectionMenu && !collectionMenu.dataset[NODE_PROCESSED_DATA_KEY]) {
    ...
    injectReact(<ComputingButtons type="items" />, container)
  }
}
const throttledInjectComputingButtons = _.throttle(injectComputingButtons, 250)
